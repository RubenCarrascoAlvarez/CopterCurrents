function [calibrated_FOV_diag,size_frame, CopterCurrents_CamCalib ] = ...
          calibration_FOV(video_fname,real_distance,time_frame_sec,...
          Height_offset,camera_offset_Z,CopterCurrents_calibration_filename,...
          Height_GPS_drone)
...
%                     antenna.
%    CopterCurrents_calibration_filename: CopterCurrents camera calibration 
%                                         filename, where the FOV camera 
%                                         parameters will be stored. 
%                                         By default is empty, so the
%                                         parameters will not be stored.
%    Height_GPS_drone: GPS sensor Height in meters.
%    
%    note:
...
if exist('CopterCurrents_calibration_filename','var') == 0 
...
CopterCurrents_CamCalib.fov_diag = calibrated_FOV_diag;
CopterCurrents_CamCalib.fov_x =  2 * atand(size_X * tand(calibrated_FOV_diag/2) / sqrt(size_X^2 + size_Y^2));
CopterCurrents_CamCalib.fov_y =  2 * atand(size_Y * tand(calibrated_FOV_diag/2) / sqrt(size_X^2 + size_Y^2));
CopterCurrents_CamCalib.size_X = size_X;
CopterCurrents_CamCalib.size_Y = size_Y;
CopterCurrents_CamCalib.camera_offset_Z = 0.0;
CopterCurrents_CamCalib.source = 'Manual_FOV_calibration';


% save calibration if the CopterCurrents_calibration_filename parameter is provided.
if ~isempty(CopterCurrents_calibration_filename)
...
% display real size
[X_eq,Y_eq,IMG_eq] = gridDJIFrame_nadir(img_gray,altitude,CopterCurrents_CamCalib);
