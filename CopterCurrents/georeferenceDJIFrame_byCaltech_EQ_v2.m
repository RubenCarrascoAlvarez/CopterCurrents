function [img_monotonic_grid,conv_monotonic_grid_ST,grid_X_RW_2d,grid_Y_RW_2d] ...
    = georeferenceDJIFrame_byCaltech_EQ_v2(img,altitude,pitch,roll,CopterCurrents_CamCalib)
% Get X and Y Real world coordeantes grid from a given Image using 
% CopterCurrents_CamCalib note X and Y grid monotonic grid, and the reference is located in
% the center of the image (CopterCurrents_CamCalib.cc, principal point coordenates)
%
...
%   CopterCurrents.
%   
%   If you want to avoid external CopterCurrents libraries, then use the function
...
%   CopterCurrents_CamCalib: calibration structure from caltech toolbox or openCV
%
% Output:
...
% check image size
if size(img,1) ~= CopterCurrents_CamCalib.ny
    error('dim1: Y axis (vertical) does not match with CopterCurrents_CamCalib.ny');
end

if size(img,2) ~= CopterCurrents_CamCalib.nx
    error('dim2: X axis (horizontal) does not match with CopterCurrents_CamCalib.nx');
end
...
% KK = [CopterCurrents_CamCalib.fc(1)  CopterCurrents_CamCalib.fc(1)*CopterCurrents_CamCalib.alpha_c  CopterCurrents_CamCalib.cc(1);...
%            0                CopterCurrents_CamCalib.fc(2)                         CopterCurrents_CamCalib.cc(2);...
%            0                            0                                           1];
...
while(any(isnan(X_lim)))
    ...
    [xp] = project_points3(X,om,T,CopterCurrents_CamCalib.fc,CopterCurrents_CamCalib.cc,CopterCurrents_CamCalib.kc,CopterCurrents_CamCalib.alpha_c);
    % value of Y where the pixel coordenate is 1
    % note: X is conected with the dim1 of xp
    X_lim = interp1(xp(1,:),vec,[1 CopterCurrents_CamCalib.nx]-1,'linear');
    RatAlt = RatAlt + 1;
end
X_lim = sort(X_lim);
...
while(any(isnan(Y_lim)))
    ...
    [xp] = project_points3(Y,om,T,CopterCurrents_CamCalib.fc,CopterCurrents_CamCalib.cc,CopterCurrents_CamCalib.kc,CopterCurrents_CamCalib.alpha_c);
    % value of Y where the pixel coordenate is 1
    % note: Y is conected with the dim2 of xp
    Y_lim = interp1(xp(2,:),vec,[1 CopterCurrents_CamCalib.ny]-1,'linear');
end
Y_lim = sort(Y_lim);

% create monotonic and ascending grid (Real World Coordenates)
offsetRatio = 1.0;
grid_Y_RW_1d = linspace(Y_lim(1)*offsetRatio ,Y_lim(2)*offsetRatio ,CopterCurrents_CamCalib.ny);
...
[xp] = project_points2(X,om,T,...
    CopterCurrents_CamCalib.fc,CopterCurrents_CamCalib.cc,CopterCurrents_CamCalib.kc,CopterCurrents_CamCalib.alpha_c);

% [xp,dxpdom,dxpdT,dxpdf,dxpdc,dxpdk] = project_points(X,om,T,f,c,k)
% [xp] = project_points(X,om,T,CopterCurrents_CamCalib.fc,CopterCurrents_CamCalib.cc,CopterCurrents_CamCalib.kc);
...
pixels_2D_X(pixels_2D_X<1 | pixels_2D_X >CopterCurrents_CamCalib.nx) = NaN;
pixels_2D_Y(pixels_2D_Y<1 | pixels_2D_Y >CopterCurrents_CamCalib.ny) = NaN;
