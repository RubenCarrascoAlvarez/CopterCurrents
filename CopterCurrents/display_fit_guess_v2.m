...
%   applied in a specific winow from IMG_SEQ. This function display 5 plots:
...
function h = update_spectra_plot(h)
    ...
    signal = Spectrum.power_Spectrum(DR_3D_mask);
    noise = Spectrum.power_Spectrum(DR_3D_mask==0);
    SNR_density = (nansum(signal)/sum(isfinite(signal))) / (nansum(noise)/sum(isfinite(noise)));
    ...
    % plot(K_cut,w_DS,'k');
    plot(K_cut_merged,w_DS_merged,'.k');
    title({['Filter width: ' num2str(w_width) ' [rad/sec]'], str_manual_current});
