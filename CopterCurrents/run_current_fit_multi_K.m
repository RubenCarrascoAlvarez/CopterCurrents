function [STCFIT] = run_current_fit_multi_K(IMG_SEQ,STCFIT,window_num_vec)
...
%      
%    window_num_vec: vector with the number of windows to analyze. If the
%    value is empy or does not exist window_num_vec = [1:STCFIT.Windows.N_fit_windows]
%         
%
%   Ouput:    
...
if exist('window_num_vec','var') == 0 || isempty(window_num_vec)
    window_num_vec =  1:STCFIT.Windows.N_fit_windows;
end

out_fit = [];

for i1 = 1:length(window_num_vec)
    
    % display window fit number
    disp([num2str(i1) ' Window N' num2str(window_num_vec(i1)) ' of ' num2str(length(window_num_vec))]);
    
    % cut i1 window
    IMG_SEQ_Window = double(IMG_SEQ.IMG(...
        STCFIT.Windows.w_corners_dim1(2,window_num_vec(i1)):STCFIT.Windows.w_corners_dim1(3,window_num_vec(i1)),...
        STCFIT.Windows.w_corners_dim2(2,window_num_vec(i1)):STCFIT.Windows.w_corners_dim2(3,window_num_vec(i1)),:));
    ...
    % fit Spectrum
    out_fit_i =  fit_Spectrum2dispersionRelation_multi_K(Spectrum,STCFIT.fit_param,STCFIT.Windows.average_depth_win(i1),window_num_vec(i1));
